...
export function registerObjectStorageRoutes(app: Express): void {
   ...
   * 
   * This uploads the file through the server to object storage.
   */
  ...
  app.get("/objects/{*path}", async (req, res) => {
    try {
      // Express 5 wildcard returns an array of path segments
      const pathParam = (req.params as unknown as { path: string | string[] }).path;
      // Join array with slashes if it's an array, otherwise use as-is
